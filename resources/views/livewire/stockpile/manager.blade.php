        ...
        <div class="flex items-center gap-2">
            <button wire:click="exportCsv" class="inline-flex items-center gap-2 rounded-md bg-zinc-700 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-zinc-600">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm.75-11.25a.75.75 0 0 0-1.5 0v4.59L7.3 9.24a.75.75 0 0 0-1.1 1.02l3.25 3.5a.75.75 0 0 0 1.1 0l3.25-3.5a.75.75 0 0 0-1.1-1.02l-1.95 2.1V6.75Z" clip-rule="evenodd" />
                </svg>
                Export Report
            </button>
        ...
        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
            <thead class="bg-zinc-50 dark:bg-zinc-800">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-300">Value (Local Currency)</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-300">In Stock</th>
                ...
                @forelse ($weapons as $weapon)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-zinc-500 dark:text-zinc-300">
                            {{ number_format($weapon->display_price, 2) }} {{ auth()->user()->country->currency_code }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-zinc-900 dark:text-white">{{ $weapon->pivot->quantity }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            ...
                            @else
                                <span class="text-xs text-zinc-400">Mfr: {{ $weapon->country->name }}</span>
                            @endcan
                        </td>
                    </tr>
                @empty
